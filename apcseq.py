...
from sc3.synth.all import Synth, Buffer
...
class Sequencer:
    ...
    def tick(context):
        ...
        while True:
            ...
            sequencer.current_step = (sequencer.current_step + 1) % sequencer.total_steps
            yield 1 / sequencer.steps_per_beat

    def __init__(self, clock, base_note=36, steps_per_beat=2, total_pages=2, light_steps=True):
        self.apc = APCMini()
        self.clock = clock
        ...
        # Light up page navigation buttons
        # self.apc.light(self.apc.bottom_row[2], "green")  # Previous page
        # self.apc.light(self.apc.bottom_row[3], "green")  # Next page

    def handle_input(self, control, value):
        if isinstance(control, Button):
            if control in self.apc.grid and value:
                ...
                for col_idx, col in enumerate(self.apc.grid_columns):
                    if control in col:
                        ...
                        self.sequence[abs_col][row_idx] = not self.sequence[abs_col][row_idx]
                        # Update button light
                        self.apc.light(control, "orange" if self.sequence[abs_col][row_idx] else "off")
                        break
            elif control == self.apc.bottom_row[2] and value:  # Previous page
                self.change_page(-1)
            elif control == self.apc.bottom_row[3] and value:  # Next page
                    self.change_page(1)
    ...
    def change_page(self, delta):
        ...
        if new_page != self.current_page:
            ...
            self.apc.light(self.apc.bottom_row[4 + new_page], "green")
            ...
            for col_idx, column in enumerate(self.apc.grid_columns):
                ...
                for row_idx, button in enumerate(column):
                    if button.lit in ["green", "red"]:
                        ...
                        self.apc.light(button, "orange" if self.sequence[abs_col][row_idx] else "off")
